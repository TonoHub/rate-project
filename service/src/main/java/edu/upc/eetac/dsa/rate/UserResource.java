...
import edu.upc.eetac.dsa.rate.dao.UserAlreadyExistsException;
...
public class UserResource {
    ...
    public Response registerUser(@FormParam("loginid") String loginid, @FormParam("password") String password, @FormParam("email") String email, @Context UriInfo uriInfo) throws URISyntaxException {
        ...
        try{
            user = userDAO.createUser(loginid, password, email);
        ...
        return Response.created(uri).type(RateMediaType.RATE_AUTH_TOKEN).entity(authenticationToken).build();
    }

    @Path("/{id}")
    @GET
    @Produces(RateMediaType.RATE_USER)
    ...
    @Path("/{id}")
    @PUT
    @Consumes(RateMediaType.RATE_USER)
    @Produces(RateMediaType.RATE_USER)
    public User updateUser(@PathParam("id") String id, User user) {
        ...
        UserDAO userDAO = new UserDAOImpl();
        try {
            user = userDAO.updateProfile(userid, user.getEmail());
            if(user == null)
                throw new NotFoundException("User with id = "+id+" doesn't exist");
        } catch (SQLException e) {
            throw new InternalServerErrorException();
